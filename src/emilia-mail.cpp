...
void Server::onBeginSlaveTask(Slave *slave) {
	Rain::Networking::Smtp::Response res(220, "emilia ready");
	slave->send(&res);
}
void Server::onRequest(Request *req) {
	...
	if (req->verb == "EHLO" || req->verb == "HELO") {
		...
		for (auto it : req->slave->data.rcptTo) {
			...
			if ((result = client.connectDomain(domain))) {
				...
				std::lock_guard<std::mutex> coutLck(coutMtx);
				std::cout << "[" << req->slave->getNativeSocket()
									<< "] Failed to send email from " << req->slave->data.mailFrom
		...
		do {
			...
			Rain::Algorithm::cStrSearchKMP(
				req->slave->data.buffer, recvLen, CRLF, 2, PART_MATCH_CRLF, &kmpCand) ==
			NULL);
